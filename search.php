...
<html lang="en">
	...
	<body class="bg-light">
		...
		<div class="container mt-5 ">
			<div class="row">
        <?php
            if(isset($_POST) and $_POST['submit'] == "Search" )
            {
		        ...
		        if($con){
                    ...
                    // Select Collection
			        $items = $db->items;
                    $qry = array("name" => $searchText);
                    $result = $items->findOne($qry);

                    if($result){
                        $cursor = $items->find($qry);
                        foreach ($cursor as $item){
                            echo '		<div class="col-sm mt-3">';
                            echo '			<div class="card bg-white text-dark">';
                            echo '				<div class="card-body">';
                            echo '					<img class="item-img" src="'.$item['img-url'].'" width="210" height="210">';
                            echo '					<br/><br/>';
                            echo '					<h5 class="card-title item-name">'.$item['name'].'</h5>';
                            echo '					<h5 class="card-title">'.$item['description'].'</h5>';
                            echo '					<h5 class="card-title item-price">'.$item['price'].'€</h5>';
                            $t =$item['stores'];
                            foreach($t as $store){
                                echo $store["store"]." ";
							...
							echo '<button onclick="addToCart(\''.$item['_id'].'\')" type="button" name="cart" id="cart" class="btn btn-block text-white mt-3 cartBtn" value="cart" style="background-color: #ff9900;">Προσθήκη στο καλάθι</button>';
                            echo '<p id="demo"></p>';
                            echo '				</div>';
                            echo '			</div>';
                            echo '		</div>';
                        ...
                        echo 'Το προιον που αναζειτηται δεν ειναι διαθεσιμο στο καταστημα μας';
